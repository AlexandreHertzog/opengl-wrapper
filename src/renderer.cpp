...
void Renderer::addVertices(const std::vector<float> &vertices, const std::vector<unsigned int> &indices) {
    vertex_arrays_.bind(0);

    vertex_buffer_.bind(0, GL_ARRAY_BUFFER);
    vertex_buffer_.buffer(static_cast<GLsizeiptr>(vertices.size() * sizeof(float)), vertices.data(), GL_STATIC_DRAW);

    if (!indices.empty()) {
        vertex_buffer_.bind(1, GL_ELEMENT_ARRAY_BUFFER);

        vertex_buffer_.buffer(static_cast<GLsizeiptr>(indices.size() * sizeof(unsigned int)), indices.data(),
                              GL_STATIC_DRAW);
    }
...
void Renderer::draw(GLenum mode) { //  NOLINT(*-function-const)
