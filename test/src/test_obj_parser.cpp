...
#include "game-engine/parsers/obj_parser.h"
...
TEST(obj_parser_test, parse) {
    ...
    game_engine::obj_parser_t parser("./untitled.obj");

    std::vector<glm::vec3> vertices;
    std::vector<glm::vec2> texture_coords;
    std::vector<std::vector<game_engine::face_t>> faces;
    std::string name;
    bool smooth = true;

    while (parser.is_good()) {
        const auto type = parser.line_type();
        if (type == game_engine::obj_parser_t::line_type_t::object_name) {
            parser.get_line(name);
        } else if (type == game_engine::obj_parser_t::line_type_t::vertex) {
            parser.get_line(vertices);
        } else if (type == game_engine::obj_parser_t::line_type_t::face) {
            parser.get_line(faces);
        } else if (type == game_engine::obj_parser_t::line_type_t::texture_coordinate) {
            parser.get_line(texture_coords);
        } else if (type == game_engine::obj_parser_t::line_type_t::smoothing) {
    ...
    game_engine::face_t f00{1, 1, 1};
    game_engine::face_t f02{7, 3, 1};
    game_engine::face_t f10{4, 5, 0};
    game_engine::face_t f12{7, 6, 0};
    game_engine::face_t f20{8, 0, 3};
    game_engine::face_t f22{5, 0, 3};
    game_engine::face_t f30{6, 0, 0};
    game_engine::face_t f32{4, 0, 0};
