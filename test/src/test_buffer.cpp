...
TEST(BufferTest, buffer_build_2) {
    ...
    auto buffers = opengl_wrapper::buffer_t::build(2);
...
TEST(BufferTest, buffer_build_0) {
    opengl_wrapper_test::mock_graphics graphics;
    EXPECT_DEATH(opengl_wrapper::buffer_t::build(0), "amount > 0");
}
...
TEST(BufferTest, buffer_move_assignment_operator) {
    ...
    opengl_wrapper::buffer_t dest(1);
...
TEST(BufferTest, buffer_bind) {
    ...
    auto buffer = opengl_wrapper::buffer_t(1, 2);
    buffer.bind();
}
